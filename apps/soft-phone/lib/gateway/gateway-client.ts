...
/**
 * Process SDP for incoming calls
 * Prefers H264 but does NOT force Baseline profile
 * Uses whatever H264 profile the remote peer sends
 */
function processSDPForIncoming(sdp: string, bitrateKbps?: number): string {
  let processed = sdp;

  // Prefer H264 codec (required for gateway compatibility)
  // but do NOT force Baseline profile - use remote peer's profile
  processed = preferH264InSdp(processed);

  // Limit incoming video bandwidth to configured kbps
  processed = limitIncomingVideoBandwidth(
    processed,
    bitrateKbps ?? VIDEO_BITRATE_KBPS,
  );
  ...
  console.log(
    '[Gateway] 📝 SDP processed for incoming call (H264 preferred, no profile forcing)',
  );
  return processed;
}
...
export class GatewayClient {
  private static readonly MAX_TRUNK_REDIRECT_ATTEMPTS = 3;
  ...
  private pendingTrunkResolvePayload: TrunkResolveMessage | null = null;
  private isTrunkResolving = false;
  private trunkResolveAttempt = 0;
  ...
  async connect(serverUrl?: string): Promise<void> {
    ...
    const url = serverUrl || settings.gatewayServer;
  ...
  disconnect(): void {
    ...
    this.pendingTrunkResolvePayload = null;
  ...
  async register(config?: Partial<GatewayConfig>): Promise<void> {
    ...
    const settings = getSettingsSync();
    this.config = {
      serverUrl: settings.gatewayServer,
  ...
  resolveTrunk(credentials: {
    ...
    const hostOnlyDomain = stripPortFromDomain(credentials.sipDomain.trim());
  ...
  async answer(): Promise<void> {
    ...
    try {
      ...
      const bitrateKbps = this.getEffectiveVideoBitrateKbps();
      ...
      // Start keyframe retry for fast video start
      this.startKeyframeRetry();
    } catch (error) {
      console.error('[Gateway] Answer failed:', error);
  ...
  private async forceNewSessionForRecovery(): Promise<void> {
    const targetUrl = this.lastServerUrl || getSettingsSync().gatewayServer;
    this.disconnect();
    this.isManualDisconnect = false;
    ...
    try {
      ...
      switch (message.type) {
        ...
        case 'incoming': {
          ...
          const incomingMsg = message as import('./types').IncomingCallResponse;
          const caller = incomingMsg.caller || incomingMsg.from || 'Unknown';
          console.log(
            '[Gateway] 📲 Incoming call from:',
            caller,
            'sessionId:',
        ...
        case 'registerStatus': {
          ...
          // Forward registerStatus to callback for trunk ID detection
          this.callbacks.onRegisterStatus?.(statusMsg);
        ...
        case 'trunk_redirect': {
          const redirectMsg =
            message as import('./types').TrunkRedirectResponse;
          console.log('[Gateway] 🔀 Trunk redirect:', redirectMsg.redirectUrl);
          void this.handleTrunkRedirect(redirectMsg.redirectUrl);
          break;
        }

        case 'trunk_not_found': {
          const notFoundMsg =
            message as import('./types').TrunkNotFoundResponse;
          console.log('[Gateway] ❌ Trunk not found:', notFoundMsg.reason);
          this.finishTrunkResolveFailure(
            notFoundMsg.reason || 'Trunk not found',
            'trunk_not_found',
          );
          break;
        }
        ...
        case 'trunk_not_ready': {
          const notReadyMsg =
            message as import('./types').TrunkNotReadyResponse;
          console.log('[Gateway] ⏳ Trunk not ready:', notReadyMsg.reason);
          this.finishTrunkResolveFailure(
            notReadyMsg.reason || 'Trunk not ready',
            'trunk_not_ready',
          );
          break;
        }
  ...
  private async handleIncomingCall(
    ...
    if (sdp && this.pc) {
      try {
        ...
        await this.pc.setRemoteDescription(remoteDesc);
        console.log('[Gateway] ✅ Remote description set for incoming call');
      } catch (error) {
        console.error(
          '[Gateway] ❌ Failed to set remote description for incoming call:',
          error,
        );
      }
    }
  ...
  private async handleTrunkRedirect(redirectUrl?: string): Promise<void> {
    this.trunkResolveAttempt++;

    if (this.trunkResolveAttempt > GatewayClient.MAX_TRUNK_REDIRECT_ATTEMPTS) {
      console.error('[Gateway] ❌ Max trunk redirect attempts exceeded');
      this.finishTrunkResolveFailure(
        'Max redirect attempts exceeded',
        'trunk_redirect',
      );
      return;
    }
    ...
    if (!redirectUrl) {
      console.error('[Gateway] ❌ Trunk redirect without URL');
      this.finishTrunkResolveFailure('Redirect without URL', 'trunk_redirect');
      return;
    }

    console.log(
      `[Gateway] 🔀 Trunk redirect (attempt ${this.trunkResolveAttempt}):`,
      redirectUrl,
    );
    this.callbacks.onTrunkRedirect?.(redirectUrl);

    // Reconnect to the redirect URL and retry trunk resolve
    try {
      this.disconnect();
      this.isManualDisconnect = false;
      await this.connect(redirectUrl);

      // Retry trunk resolve on new connection
      if (this.pendingTrunkResolvePayload) {
        console.log(
          '[Gateway] 🔄 Retrying trunk resolve on redirected server...',
        );
        this.send(this.pendingTrunkResolvePayload);
      }
    } catch (error) {
      console.error('[Gateway] ❌ Failed to connect to redirect URL:', error);
      this.finishTrunkResolveFailure(
        `Failed to connect to ${redirectUrl}`,
        'trunk_redirect',
      );
    }
  }

  private finishTrunkResolveFailure(
    reason: string,
    type: 'trunk_not_found' | 'trunk_not_ready' | 'trunk_redirect' | 'error',
  ): void {
    this.isTrunkResolving = false;
    this.callbacks.onTrunkResolveFailed?.(reason, type);
  }

  private getEffectiveVideoBitrateKbps(): number {
    const settings = getSettingsSync();
    const profileBitrate = getBitrateForResolution(settings.videoResolution);
    const bitrateKbps = Math.min(profileBitrate, VIDEO_BITRATE_KBPS);
