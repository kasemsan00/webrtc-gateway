...
export function Dialpad({
  ...
  const { keySize, gap, mainFontSize, subFontSize, letterSpacing } =
    useMemo(() => {
      ...
      const widthBasedKeySize = Math.floor(
        (availableWidth - clampedGap * 2) / 3,
      );

      // 3. Calculate Key Size based on HEIGHT (4 rows)
      // Formula: (Height - gap*3) / 4
      const heightBasedKeySize = Math.floor(
        (availableHeight - clampedGap * 3) / 4,
      );
      ...
      const clampedKeySize = Math.max(
        MIN_KEY,
        Math.min(maxKeySize, rawKeySize),
      );
      ...
      const rawLetterSpacing = Math.max(
        1,
        Math.min(3, clampedSubFontSize * 0.12),
      );
    ...
    }, [layout.width, layout.height, windowWidth, windowHeight]);
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      onMetricsChange({ keySize, gap });
    }, 0);

    return () => {
      clearTimeout(timeoutId);
    };
  }, [gap, keySize, onMetricsChange]);
      ...
      {KEYS.map((row, rowIndex) => (
        <View key={rowIndex} style={[styles.row, { gap }]}>
          {row.map((key) => (
            ...
            >
              <Text style={[styles.keyMain, { fontSize: mainFontSize }]}>
                {key.main}
              </Text>
              {key.sub ? (
                <Text
