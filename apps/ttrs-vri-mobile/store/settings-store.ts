...
const DEFAULT_SETTINGS: SettingsState = {
  // SIP credentials
  sipDomain: "203.150.245.41",
  sipUsername: "00022",
  sipPassword: "test1234",
  sipDisplayName: "00022",
  sipPort: 5060,
...
export const useSettingsStore = create<SettingsStore>()(
  persist(
    (set, get) => ({
      ...
      // SIP credential setters
      setSipDomain: (value) => set({ sipDomain: value }),
      setSipUsername: (value) => set({ sipUsername: value }),
      setSipPassword: (value) => set({ sipPassword: value }),
      setSipDisplayName: (value) => set({ sipDisplayName: value }),
      setSipPort: (value) => set({ sipPort: value }),
      ...
      // Check if settings are complete
      hasRequiredSettings: () => {
        const { sipDomain, sipUsername, sipPassword } = get();
        return !!(sipDomain && sipUsername && sipPassword);
      },
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          ...
          // Remove legacy gatewayServer field (now env-only)
          // @ts-ignore - remove legacy gatewayServer field
          if (state.gatewayServer !== undefined) {
            console.log("[SettingsStore] Removing legacy gatewayServer field (env-only)");
            // @ts-ignore
            delete state.gatewayServer;
          }
        }
      },
    },
...
export function getGatewayServerFromEnv(): string {
  const envValue = process.env.EXPO_PUBLIC_GATEWAY_SERVER?.trim();
  if (envValue) {
    return envValue;
  }
