<?php
  ...
  $strSQL = "SELECT * FROM `log_send_popup` WHERE `uniqueid` >= '1716778598.1754' AND `uniqueid` <= '1716778757.1830'";
  // echo $strSQL;

  $results = dbquery($strSQL, DB_FETCH_ALL);

  foreach ($results as $key => $result) {
    $strSQL = "SELECT * FROM `log_send_popup` WHERE `uniqueid` = $result[uniqueid] AND `finish` = 'true'";
