...
func (s *Server) handleBYE(req *sip.Request, tx sip.ServerTransaction) {
	...
	if sess != nil {
		...
		if s.stateNotifier != nil {
			s.stateNotifier.NotifySessionState(sess.ID, session.StateEnded)
	...
	if s.sessionMgr != nil && callIDValue != "" {
		if sess, ok := s.sessionMgr.GetSessionBySIPCallID(callIDValue); ok {
			...
			if !sess.HasDialogState() {
				fmt.Printf("📞 [Session %s] Setting dialog state from ACK (fallback)\n", sess.ID)
				...
				if fromHeader := req.From(); fromHeader != nil {
					...
					fmt.Printf("📞 [Session %s] ACK From (remote party): %s\n", sess.ID, remotePartyURI)
				}
				...
				if toHeader := req.To(); toHeader != nil {
					...
					fmt.Printf("📞 [Session %s] ACK To (local party): %s\n", sess.ID, localPartyURI)
				}
				...
				if inviteReq, ok := sess.GetIncomingInviteRequest().(*sip.Request); ok && inviteReq != nil {
					...
					fmt.Printf("📞 [Session %s] Original INVITE Record-Route headers: %d found\n", sess.ID, len(recordRoutes))
					...
					fmt.Printf("📞 [Session %s] ⚠️ No stored INVITE request to extract Record-Route\n", sess.ID)
				}
				...
				fmt.Printf("✅ [Session %s] Dialog state set from ACK - FromTag: %s, ToTag: %s, Contact: %s\n",
					sess.ID, toTag, fromTag, remoteContact)
				fromValue, toValue := sess.GetFromTo()
				fmt.Printf("✅ [Session %s] Updated URIs - From: %s, To: %s\n", sess.ID, fromValue, toValue)
			}
		}
	}
}
...
func (s *Server) handleSwitchMessage(body string, callerURI string) {
	...
	// Enable RTCP fallback window after @switch
	sess.StartVideoRTCPFallbackWindow(4*time.Second, "@switch")

	// 4. Send FIR first (3 times) to request full keyframe with parameter sets (SPS/PPS)
	fmt.Printf("[Session %s] 🔀 Sending @switch: FIR requests (3x) for fast video start with SPS/PPS\n", sess.ID)
	for i := 0; i < 10; i++ {
		if sess.GetState() == session.StateEnded {
			return
		}
	...
	fmt.Printf("[Session %s] 🔀 Sending @switch: PLI requests (10x) for fast video start\n", sess.ID)
	for i := 0; i < 10; i++ {
		if sess.GetState() == session.StateEnded {
			return
		}
		sess.SendPLItoWebRTC()   // PLI to browser
		sess.SendPLIToAsterisk() // PLI to Asterisk
		time.Sleep(100 * time.Millisecond)
	}
