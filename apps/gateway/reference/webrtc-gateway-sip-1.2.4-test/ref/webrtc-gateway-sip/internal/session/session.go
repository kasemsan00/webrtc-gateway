...
type Session struct {
	...
	AsteriskVideoRTCPAddr  *net.UDPAddr `json:"-"` // Learned RTCP address from actual traffic
	...
	// Cached SPS/PPS for video injection
	CachedSPS               []byte    `json:"-"`
	CachedPPS               []byte    `json:"-"`
	...
	SIPDomain        string   `json:"-"` // SIP domain
...
func NewSession(id string, cfg *config.Config, turnConfig config.TURNConfig) (*Session, error) {
		...
		for {
			...
			for _, p := range packets {
				...
				case *rtcp.PictureLossIndication:
					fmt.Printf("[Session %s] 📸 Received PLI from browser for video (RTPSender)\n", id)
					session.SendPLIToAsterisk() // Forward to Asterisk
				case *rtcp.FullIntraRequest:
					fmt.Printf("[Session %s] 📸 Received FIR from browser for video (RTPSender)\n", id)
					session.SendFIRToAsterisk() // Forward to Asterisk
				case *rtcp.TransportLayerNack:
					fmt.Printf("[Session %s] 🔄 Received NACK from browser for video (RTPSender) - Nacks=%v\n", id, pkt.Nacks)
					session.SendNACKToAsterisk(pkt.Nacks) // Forward to Asterisk
				}
			}
		}
	}()
	...
	if cfg.API.DebugTURN {
		...
		peerConnection.OnICEGatheringStateChange(func(state webrtc.ICEGatheringState) {
			fmt.Printf("[Session %s] 🧊 ICE Gathering State: %s\n", id, state.String())
		})
		...
		peerConnection.OnICECandidate(func(candidate *webrtc.ICECandidate) {
			if candidate == nil {
				...
				fmt.Printf("[Session %s] 🧊 ICE Candidate gathering complete\n", id)
				return
			}
	...
	peerConnection.OnICEConnectionStateChange(func(connectionState webrtc.ICEConnectionState) {
		...
		case webrtc.ICEConnectionStateConnected:
			...
			if cfg.API.DebugTURN {
				...
				if selectedPair != nil {
					...
					if localCandidate != nil && remoteCandidate != nil {
						...
						fmt.Printf("[Session %s] 🧊 Selected Candidate Pair:%s\n", id, turnIndicator)
			...
			if session.State == StateConnecting || session.State == StateReconnecting {
				...
				if wasReconnecting {
					fmt.Printf("[Session %s] ✅ ICE reconnected - resuming call\n", id)
				}
			}
			// Send FIR first (to request SPS/PPS + IDR), then PLI burst for fast video start
			go func() {
				fmt.Printf("[Session %s] 🚀 ICE Connected - Sending FIR + PLI requests for fast video start (with SPS/PPS)\n", id)
				...
				for i := 0; i < 3; i++ {
					time.Sleep(100 * time.Millisecond)
					session.SendPLIToAsterisk() // PLI to Asterisk
					session.SendPLItoWebRTC()   // PLI to browser
				}
			}()

		case webrtc.ICEConnectionStateDisconnected, webrtc.ICEConnectionStateClosed:
			// Don't immediately end - give time for network recovery
			if session.State == StateActive {
				session.State = StateReconnecting
				fmt.Printf("[Session %s] 📡 ICE %s - entering reconnection grace period (30s)\n", id, connectionState.String())
				...
				go func() {
					...
					// If still in reconnecting state after grace period, end the session
					if session.State == StateReconnecting {
						fmt.Printf("[Session %s] ⏰ Grace period expired - ending session\n", id)
			...
			} else if session.State != StateReconnecting {
				// If not active and not already reconnecting, end immediately
				session.State = StateEnded
				if session.RTPConn != nil {
					session.RTPConn.Close()
					session.RTPConn = nil
					...
					if strings.HasPrefix(param, "sprop-parameter-sets=") {
						...
						if len(sets) >= 1 {
							...
							if err == nil {
								...
								session.mu.Unlock()
								fmt.Printf("[Session %s] 💾 Cached SPS from SDP (%d bytes)\n", id, len(sps))
							}
						}
						if len(sets) >= 2 {
							...
							if err == nil {
								...
								session.mu.Unlock()
								fmt.Printf("[Session %s] 💾 Cached PPS from SDP (%d bytes)\n", id, len(pps))
							}
						}
					}
				}
		...
		go func() {
			...
			for {
				...
				for _, p := range packets {
					switch pkt := p.(type) {
					case *rtcp.PictureLossIndication:
						fmt.Printf("[Session %s] 📸 Received PLI from browser for %s\n", id, kind)

						// Forward PLI to Asterisk (Only for Video)
						if kind == "video" {
							session.SendPLIToAsterisk()
						}

					case *rtcp.FullIntraRequest:
						fmt.Printf("[Session %s] 📸 Received FIR from browser for %s\n", id, kind)
						if kind == "video" {
							session.SendFIRToAsterisk()
						}

					case *rtcp.TransportLayerNack:
						fmt.Printf("[Session %s] 🔄 Received NACK from browser for %s - Nacks=%v\n", id, kind, pkt.Nacks)
						if kind == "video" {
...
func (s *Session) SetSIPDialogState(fromTag, toTag, remoteContact, domain string, port, cseq int, routeSet []string) {
	...
	s.UpdatedAt = time.Now()
	fmt.Printf("[Session %s] SIP Dialog state set - FromTag: %s, ToTag: %s, Domain: %s, RouteSet: %v\n", s.ID, fromTag, toTag, domain, s.SIPRouteSet)
}
...
func (s *Session) WaitForSPSPPS(timeout time.Duration) bool {
	...
	if s.HasCachedSPSPPS() {
		fmt.Printf("[Session %s] ✅ SPS/PPS already cached (immediate check)\n", s.ID)
		return true
	}

	fmt.Printf("[Session %s] ⏳ Waiting up to %v for SPS/PPS to be cached...\n", s.ID, timeout)
	...
	for {
		...
		case <-ticker.C:
			if s.HasCachedSPSPPS() {
				...
				s.mu.RUnlock()
				fmt.Printf("[Session %s] ✅ SPS/PPS cached (SPS: %d bytes, PPS: %d bytes)\n", s.ID, spsLen, ppsLen)
				return true
			}
		case <-timeoutChan:
			fmt.Printf("[Session %s] ⚠️ SPS/PPS not ready after %v; proceeding without sprop-parameter-sets\n", s.ID, timeout)
			return false
		}
	}
}
