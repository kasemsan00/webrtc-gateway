...
func (s *Server) createSDPOffer(rtpPort int, sess *session.Session) []byte {
	...
	if forceAVP == "true" || forceAVP == "1" || forceAVP == "yes" {
		...
		videoProfile = "RTP/AVP"
		rtcpFbLines = ""
		fmt.Printf("[Session %s] ⚠️ Forcing RTP/AVP to SIP side (SIP_FORCE_AVP=%s)\n", sess.ID, forceAVP)
	}
	...
	if sps, pps, ok := sess.GetCachedSPSPPS(); ok {
		...
		fmt.Printf("[Session %s] 🎬 Including sprop-parameter-sets in SDP (SPS: %d bytes, PPS: %d bytes)\n", sess.ID, len(sps), len(pps))
	}
...
%s
%sa=sendrecv
`, s.getActiveUsername(), sessionID, sessionID, s.publicAddress,
		s.publicAddress,
...
func (s *Server) parseAsteriskSDPAndSetEndpoints(sdpBody []byte, sess *session.Session) {
	...
	if asteriskIP != "" && audioPort > 0 {
		...
		fmt.Printf("[Session %s] ✅ Asterisk RTP endpoints configured - Audio: %s:%d, Video: %s:%d\n",
			sess.ID, asteriskIP, audioPort, asteriskIP, videoPort)

		// Check if video was rejected (port=0)
		if videoPort == 0 {
			fmt.Printf("[Session %s] ❌ CRITICAL: Asterisk rejected video (port=0) - Video stream will not work!\n", sess.ID)
			if s.config.VideoUseAVPF {
				fmt.Printf("[Session %s] ⚠️ AVPF was offered but Asterisk rejected video\n", sess.ID)
				fmt.Printf("[Session %s] 📋 Full SDP Answer from Asterisk (for debugging):\n%s\n", sess.ID, fullSDP)
				fmt.Printf("[Session %s] 💡 TROUBLESHOOTING: Check Asterisk configuration:\n", sess.ID)
				fmt.Printf("[Session %s]    - For res_pjsip: Set 'use_avpf=yes' in endpoint config\n", sess.ID)
				fmt.Printf("[Session %s]    - For chan_sip: Set 'videosupport=yes' in peer config\n", sess.ID)
				fmt.Printf("[Session %s]    - Ensure H.264 codec is enabled: 'allow=h264' or 'allow=!all,allow=h264'\n", sess.ID)
				fmt.Printf("[Session %s]    - Verify Asterisk version supports AVPF (Asterisk 13+ recommended)\n", sess.ID)
				fmt.Printf("[Session %s]    - Check Asterisk logs: 'asterisk -rvvv' or 'core show settings'\n", sess.ID)
			} else {
				fmt.Printf("[Session %s] 📋 Full SDP Answer from Asterisk (for debugging):\n%s\n", sess.ID, fullSDP)
				fmt.Printf("[Session %s] 💡 TROUBLESHOOTING: Check Asterisk configuration:\n", sess.ID)
				fmt.Printf("[Session %s]    - For res_pjsip: Set 'videosupport=yes' in endpoint config\n", sess.ID)
				fmt.Printf("[Session %s]    - For chan_sip: Set 'videosupport=yes' in peer config\n", sess.ID)
				fmt.Printf("[Session %s]    - Ensure H.264 codec is enabled: 'allow=h264' or 'allow=!all,allow=h264'\n", sess.ID)
			}
		}
		...
		if s.config.VideoUseAVPF {
			if videoPort > 0 {
				...
				if videoProfile == "RTP/AVPF" {
					if hasRtcpFb {
						fmt.Printf("[Session %s] ✅ AVPF negotiation successful - Asterisk accepted RTP/AVPF with RTCP feedback\n",
							sess.ID)
					} else {
						fmt.Printf("[Session %s] ⚠️ AVPF profile accepted but no RTCP feedback attributes found in answer (payload 96)\n",
							sess.ID)
						fmt.Printf("[Session %s] 💡 Asterisk may not support RTCP feedback - video will work but PLI/FIR/NACK may not function\n",
							sess.ID)
					}
				} else if videoProfile != "" {
					fmt.Printf("[Session %s] ⚠️ AVPF was offered but Asterisk answered with %s (AVPF not negotiated, using %s)\n",
						sess.ID, videoProfile, videoProfile)
					fmt.Printf("[Session %s] 💡 Video will work but RTCP feedback (PLI/FIR/NACK) will not be available\n",
						sess.ID)
				}
			}
		}
	} else {
		fmt.Printf("[Session %s] ⚠️ Failed to parse Asterisk SDP endpoints (IP: %s, Audio: %d, Video: %d)\n",
