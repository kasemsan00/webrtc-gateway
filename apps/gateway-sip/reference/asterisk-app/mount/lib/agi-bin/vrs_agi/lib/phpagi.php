...
class AGI {
    ...
    function stream_multifile($filenames, $escape_digits='', $offset=0, $startfile=0) {
	//for($i=$startfile;$i<count($filenames);$i++){
	$i = 0;
	foreach ($filenames as $file) {
	    if ($i >= $startfile) {
		$press = $this->stream_file($file, $escape_digits, $offset);
		if ($press[result] != '0') {
		    $press[wav] = $file;
		    $press[ifile] = $i;
		    return $press;
