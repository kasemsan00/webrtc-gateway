<?php
  ...
  //$strSQL = "SELECT * FROM `vrslog`.`log_send_popup` WHERE `http_code` != '200' AND `finish` = 'true' AND `create_at` LIKE '2022-12-05 1%'";
  //$strSQL = "SELECT * FROM `vrslog`.`log_send_popup` WHERE `http_code` != '200' AND `finish` = 'true' AND `uniqueid` = '1677065136.497462'";
  $strSQL = "SELECT * FROM `vrslog`.`log_send_popup` WHERE `uniqueid` IN (SELECT `uniqueid` FROM `vrslog`.`log_send_popup` WHERE `http_code` != '200' AND `finish` = 'true' AND `action_at` > '1709405913.32719') ORDER BY `uniqueid`";
  echo $strSQL;
  ...
  foreach ($results as $key => $result) {
    ...
    $strSQL = "SELECT * FROM `vrslog`.`log_send_popup` WHERE `uniqueid` = $result[uniqueid] AND `http_code` = '200'";
...
//      echo date('h:i:s') . "\t" . $argv[2] . "\n";
      ...
      try {
        ...
        echo date('h:i:s') . "\t" . $argv[2] . "\t" . $response . "\n";
