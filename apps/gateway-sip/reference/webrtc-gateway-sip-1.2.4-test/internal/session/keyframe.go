...
// Allow sending when keyframe is stale OR never received, and last PLI was not too recent.
func (s *Session) shouldSendPLIToAsterisk(now time.Time) bool {
	s.mu.RLock()
	lastKeyframe := s.LastKeyframe
	lastPLISent := s.LastPLISent
	s.mu.RUnlock()

	if !lastPLISent.IsZero() && now.Sub(lastPLISent) < pliMinInterval {
		return false
	}
...
func (s *Session) SendPLIToAsterisk() {
	now := time.Now()
	if !s.shouldSendPLIToAsterisk(now) {
		fmt.Printf("[Session %s] ⏱️ Skipping PLI to Asterisk - keyframe is recent or PLI too frequent\n", s.ID)
		return
	}
