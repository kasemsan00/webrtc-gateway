...
// TrunkListResponse represents a list of trunks
...
// handleListTrunks returns all trunks from the database
func (s *Server) handleListTrunks(w http.ResponseWriter, r *http.Request) {
	...
	trunks, err := s.trunkManager.ListTrunks(r.Context())
	if err != nil {
		s.respondError(w, http.StatusInternalServerError, fmt.Sprintf("Failed to list trunks: %v", err))
		return
	}

	response := make([]TrunkResponse, 0, len(trunks))
	for _, trunk := range trunks {
		response = append(response, trunkResponseFrom(trunk))
	}

	s.respondJSON(w, http.StatusOK, TrunkListResponse{Items: response})
}
...
func trunkResponseFrom(trunk *sip.Trunk) TrunkResponse {
