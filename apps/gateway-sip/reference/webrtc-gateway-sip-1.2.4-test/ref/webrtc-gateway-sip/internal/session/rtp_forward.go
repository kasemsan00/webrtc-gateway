...
func (s *Session) forwardRTPToAsterisk(track *webrtc.TrackRemote, kind string) {
	...
	for {
		...
		if err != nil {
			...
			fmt.Printf("[Session %s] Error reading %s from WebRTC: %v\n", s.ID, kind, err)
			return
		}
		...
		if kind == "video" && len(packet.Payload) > 0 {
			...
			if nalType == 7 {
				...
				if shouldUpdate {
					...
					fmt.Printf("[Session %s] 💾 Cached SPS from RTP stream (packet #%d, %d bytes)\\n", s.ID, packetCount, len(packet.Payload))
				}
				s.mu.Unlock()
			} else if nalType == 8 {
				...
				if shouldUpdate {
					...
					fmt.Printf("[Session %s] 💾 Cached PPS from RTP stream (packet #%d, %d bytes)\\n", s.ID, packetCount, len(packet.Payload))
				}
				s.mu.Unlock()
			} else if nalType == 5 {
				isKeyframe = true
				...
				isSTAPA = true
				if packetCount <= 20 {
					fmt.Printf("[Session %s] 📦 STAP-A detected in packet #%d (%d bytes) - will de-aggregate\n", s.ID, packetCount, len(packet.Payload))
				}
		...
		if isSPS || isPPS {
			...
			if packetCount <= 50 {
				fmt.Printf("[Session %s] 📦 Forwarding %s from browser to Asterisk (packet #%d)\\n", s.ID, nalName, packetCount)
			}
		}
		...
		if kind == "audio" {
			...
			if packetCount <= 5 {
				fmt.Printf("[Session %s] 🎵 Audio packet #%d: PT=%d, payload=%d bytes\n",
			...
			if s.SIPOpusPT > 0 && s.SIPOpusPT != 111 && packet.Header.PayloadType == 111 {
				...
				if packetCount <= 5 {
					fmt.Printf("[Session %s] 🔄 Rewrite audio PT: 111 → %d (packet #%d)\n", s.ID, s.SIPOpusPT, packetCount)
				}
			}
			...
			if packetCount <= 100 && len(packet.Payload) > 0 {
				...
				fmt.Printf("[Session %s] 🔍 WebRTC Video Packet #%d: NAL Type=%d (%s), NRI=%d, Size=%d bytes\n",
			...
			if s.VideoSSRC == 0 {
				...
				go func() {
					fmt.Printf("[Session %s] 🚀 Starting video forwarding to Asterisk (SSRC=%d) - requesting keyframes from browser\n", s.ID, s.VideoSSRC)
					for i := 0; i < 10; i++ {
			...
			if isSTAPA && len(packet.Payload) > 1 {
					...
					if len(nalUnit) > 0 {
						...
						case 7:
							...
							if shouldUpdate {
								...
								if packetCount <= 50 {
									fmt.Printf("[Session %s] 💾 Cached SPS from STAP-A (%d bytes)\n", s.ID, len(nalUnit))
								}
							}
						case 8:
							containsPPS = true
							...
							if shouldUpdate {
								...
								if packetCount <= 50 {
									fmt.Printf("[Session %s] 💾 Cached PPS from STAP-A (%d bytes)\n", s.ID, len(nalUnit))
								}
							}
						case 5:
							containsIDR = true
				...
				if containsIDR {
					fmt.Printf("[Session %s] 🔑 KEYFRAME (STAP-A IDR) detected in video packet #%d → Asterisk\n", s.ID, packetCount)
				}
				...
				if shouldInjectBeforeSTAPA {
					...
					fmt.Printf("[Session %s] 💉 STAP-A contains IDR - injecting Separate SPS/PPS (NRI=3) (%s, %dx) before de-aggregation (stapaHasSPS=%v, stapaHasPPS=%v)\n",
					...
					for i := 0; i < redundancyCount; i++ {
						...
						if out, err := spsPacket.Marshal(); err == nil {
							if destAddr != nil && conn != nil {
								...
								if injectReason == "@switch" {
									fmt.Printf("[Session %s] 💉 @switch STAP-A: Injected SPS (NRI=3) copy %d/%d (Seq=%d, Size=%d)\n",
						...
						if out, err := ppsPacket.Marshal(); err == nil {
							if destAddr != nil && conn != nil {
								conn.WriteToUDP(out, destAddr)
								if injectReason == "@switch" {
									fmt.Printf("[Session %s] 💉 @switch STAP-A: Injected PPS (NRI=3) copy %d/%d (Seq=%d, Size=%d)\n",
					...
					if injectReason == "@switch" {
						...
						fmt.Printf("[Session %s] 🔀 @switch STAP-A: Injected for IDR (remaining: %d)\n", s.ID, s.SwitchSPSPPSInjectRemaining)
					}
				}
					...
					if len(nalUnit) > 0 {
						...
						if out, err := nalPacket.Marshal(); err == nil {
							if destAddr != nil && conn != nil {
								conn.WriteToUDP(out, destAddr)
								if packetCount <= 50 {
									fmt.Printf("[Session %s] 📤 De-aggregated STAP-A NAL #%d type=%d (Seq=%d, Size=%d, Marker=%v)\n",
				...
				for i := 0; i < redundancyCount; i++ {
					...
					if out, err := spsPacket.Marshal(); err == nil {
						if destAddr != nil && conn != nil {
							...
							if (isKeyframe && i == 0) || packetCount <= 100 {
								fmt.Printf("[Session %s] 💉 Injected SPS (NRI=3) copy %d/%d (Seq=%d, Size=%d, Keyframe=%v)\n",
					...
					if out, err := ppsPacket.Marshal(); err == nil {
						if destAddr != nil && conn != nil {
							conn.WriteToUDP(out, destAddr)
							if (isKeyframe && i == 0) || packetCount <= 100 {
								fmt.Printf("[Session %s] 💉 Injected PPS (NRI=3) copy %d/%d (Seq=%d, Size=%d, Keyframe=%v)\n",
		...
		if destAddr != nil && conn != nil {
			...
			if err != nil {
				fmt.Printf("[Session %s] Error marshalling RTP: %v\n", s.ID, err)
				continue
			}

			// Logging (re-using NAL type detection mostly for logging)
			if kind == "video" && isKeyframe {
				fmt.Printf("[Session %s] 🔑 KEYFRAME (Forwarded) detected in video packet #%d → Asterisk\n", s.ID, packetCount)
			}

			// Log first few forwarded packets
			if packetCount <= 5 || packetCount%500 == 0 || isKeyframe {
				fmt.Printf("[Session %s] 📤 Re-packetized %s #%d (Seq=%d, SSRC=%d, Size=%d) → %s\n",
			...
			if err != nil {
				if packetCount <= 5 {
					fmt.Printf("[Session %s] Error forwarding %s to Asterisk: %v\n", s.ID, kind, err)
				}
			}
		} else if packetCount == 1 {
			fmt.Printf("[Session %s] ⚠️ Cannot forward %s: Asterisk endpoint not set yet\n", s.ID, kind)
		}
	}
}
