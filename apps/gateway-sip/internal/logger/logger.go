...
func (l *Logger) captureOutput() {
	...
	for {
		...
		default:
			...
			if n > 0 {
				l.mu.Lock()
				l.writeCapturedWithTimestamp(buf[:n])
				l.mu.Unlock()
			}
		}
	}
...
func (l *Logger) writeCapturedWithTimestamp(chunk []byte) {
	if len(chunk) == 0 {
		return
	}
	...
	for {
		nl := bytes.IndexByte(l.lineBuf, '\n')
		if nl == -1 {
			return
		}

		line := l.lineBuf[:nl]
		l.writeLineWithTimestamp(line, true)
		l.lineBuf = l.lineBuf[nl+1:]
	}
}

func (l *Logger) writeLineWithTimestamp(line []byte, newline bool) {
	timestamp := time.Now().Format("2006-01-02 15:04:05.000")
	fmt.Fprintf(l.multiWriter, "%s ", timestamp)
	l.multiWriter.Write(line)
	if newline {
		l.multiWriter.Write([]byte{'\n'})
	}
}
...
func (l *Logger) Close() {
	...
	if len(l.lineBuf) > 0 {
		l.writeLineWithTimestamp(l.lineBuf, true)
		l.lineBuf = nil
	}
