...
func (s *Session) forwardRTPToAsterisk(track *webrtc.TrackRemote, kind string) {
	...
	for {
		...
		if kind == "video" && isSTAPA && len(packet.Payload) > 1 {
			stapaScan = analyzeSTAPAPayload(packet.Payload[1:])
		}
		...
		if kind == "audio" {
			...
			if isSTAPA && len(packet.Payload) > 1 {
				...
				containsIDR := stapaScan != nil && stapaScan.containsIDR
				containsSPS := stapaScan != nil && stapaScan.containsSPS
				containsPPS := stapaScan != nil && stapaScan.containsPPS
				if containsIDR {
					isKeyframe = true
				}
				if stapaScan != nil && len(stapaScan.cachedSPS) > 0 {
					curLen := len(s.CachedSPS)
					shouldUpdate := curLen == 0 || !bytes.Equal(s.CachedSPS, stapaScan.cachedSPS)
					if shouldUpdate {
						s.CachedSPS = append([]byte(nil), stapaScan.cachedSPS...)
						if packetCount <= 50 {
							fmt.Printf("[%s] 💾 Cached SPS from STAP-A (%d bytes)\n", s.ID, len(stapaScan.cachedSPS))
						}
					}
				}
				if stapaScan != nil && len(stapaScan.cachedPPS) > 0 {
					curLen := len(s.CachedPPS)
					shouldUpdate := curLen == 0 || !bytes.Equal(s.CachedPPS, stapaScan.cachedPPS)
					if shouldUpdate {
						s.CachedPPS = append([]byte(nil), stapaScan.cachedPPS...)
						if packetCount <= 50 {
							fmt.Printf("[%s] 💾 Cached PPS from STAP-A (%d bytes)\n", s.ID, len(stapaScan.cachedPPS))
						}
					}
				}
...
func analyzeSTAPAPayload(payload []byte) *stapaScanResult {
	result := &stapaScanResult{}
	offset := 0
	for offset+2 <= len(payload) {
		nalSize := int(payload[offset])<<8 | int(payload[offset+1])
		offset += 2
		...
		case 5:
			result.containsIDR = true
		case 7:
			result.containsSPS = true
			result.cachedSPS = append([]byte(nil), nalUnit...)
		case 8:
			result.containsPPS = true
			result.cachedPPS = append([]byte(nil), nalUnit...)
		}
	}
	return result
}
