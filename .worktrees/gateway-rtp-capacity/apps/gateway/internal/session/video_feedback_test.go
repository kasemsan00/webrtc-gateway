...
func TestAnalyzeSTAPAPayload_DetectsIDRSPSPPS(t *testing.T) {
	// STAP-A payload body: [size][nal]...
	// nal 7 (SPS), nal 8 (PPS), nal 5 (IDR)
	payload := []byte{
		0, 2, 0x67, 0x64,
		0, 2, 0x68, 0xee,
		0, 2, 0x65, 0x88,
	...
	if !got.containsIDR || !got.containsSPS || !got.containsPPS {
		t.Fatalf("expected IDR/SPS/PPS all true, got %+v", got)
	}
	if len(got.cachedSPS) == 0 || got.cachedSPS[0]&0x1F != 7 {
		t.Fatalf("expected cached SPS nal, got %v", got.cachedSPS)
	}
	if len(got.cachedPPS) == 0 || got.cachedPPS[0]&0x1F != 8 {
		t.Fatalf("expected cached PPS nal, got %v", got.cachedPPS)
	}
}
